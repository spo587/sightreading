...
def nextNoteStep(currentScaleDegree, level, pinkyDegree=4):
    ...
    if level == 1:
        ...
        increment = 2 if randnum2 < 0.25 else 1
...
def stepNoteNumsFiveFinger(rhythms, level):
    '''should make one that takes rhythms as inputs too?'''
    notes = [0] ## always end on tonic
    for i in range(1,len(rhythms)):
        notes.append(nextNoteStep(notes[i-1], level))
    notes.reverse()
    notes = [notes[i] + 1 for i in range(len(notes))] ## fix this!!
    return notes
